...
max_volume_map = {symbol: 0 for symbol in symbols}
...
while True:
    ...
    for token, data in all_quotes.items():
        ...
        if len(stock_data[token]) > 0:  
            ...
            max_volume_map[token] = max(max_volume_map[token], candle["volume_1_min"])
        ...
        if len(stock_data[token]) == 2:
            ...
            if c3["cummulative_volume"]-previousVolume>avg_volume_of_this_stock and token not in unusualVolumeSymbols:
                ...
                volumeFlag=False
                if(c3["close"]>=3000 and max_volume_map[token]>=20000):
                    volumeFlag=True
                elif (c3["close"]>=2000 and max_volume_map[token]>=25000):    
                    volumeFlag=True
                elif (c3["close"]>=1500 and max_volume_map[token]>=30000):    
                    volumeFlag=True
                elif (c3["close"]>=1000 and max_volume_map[token]>=62500):    
                    volumeFlag=True
                elif (c3["close"]>=500 and max_volume_map[token]>=180000):    
                    volumeFlag=True
                elif (c3["close"]>=250 and max_volume_map[token]>=200000):    
                    volumeFlag=True  
                elif (c3["close"]>=125 and max_volume_map[token]>=450000):    
                    volumeFlag=True 
                elif (c3["close"]>=50 and max_volume_map[token]>=1000000):    
                    volumeFlag=True
                elif (c3["close"]<50 and max_volume_map[token]>=2000000):    
                    volumeFlag=True             
                ...
                if  potential_gain<18.5 and turnover>4000 and dayHigh=="high" and volumeFlag:
                    log_momentum_signal(candle,avg_volume_of_this_stock,0,dayHigh,link,token,count)
                    unusualVolumeSymbols.add(token)
                elif potential_gain>21.5 and turnover>4000 and dayLow=="low" and volumeFlag:
